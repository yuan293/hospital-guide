...
export async function evaluateCase(row, mode, model, runner = runTriage) {
  ...
  const trace = [];
  // 一次性组（rules/model）只注入安全槽回答；部位与鉴别追问的回答属于交互流程，
  // 只有 loop 组按流程逐轮追问时才会用到，不能凭空喂给“没问过”的一次性基线。
  const slotAnswers = Object.fromEntries(Object.entries(row.input.answers).filter(([key]) => !key.startsWith('p_')));
  const answers = config.loop ? {} : { ...slotAnswers };
  ...
  return { id: row.id, suite: row.suite, category: row.category, chief: row.input.chief, skipped: false,
    ...
    // 只统计真正的调用失败（不可用/超时）；入口介入返回“没有可用证据”属于正常结果，
    // 不算失败，也不应触发退出码门禁。
    modelFailures: trace.filter(t => t.model?.failed).length,
    turns: trace.length,
    trace };
}
