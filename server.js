...
export function createApp() {
  return http.createServer(async (req, res) => {
    try {
      ...
      if (req.method === 'GET' && route === '/api/health') return json(res, 200, { version: '0.5.1', status: 'ok', pid: process.pid, privacy: '本应用不保存导诊会话，不发送到云端。', ollama: await modelStatus() });
...
export function start(port = Number(process.env.PORT || 3210)) {
  ...
  server.listen(port, '127.0.0.1', () => console.log(`Hospital Guide v0.5.1\nOpen: http://127.0.0.1:${port}\nDemo only. Ctrl+C to stop.`));
  return server;
}
