...
description: 诊途项目改动后的标准验证闭环，含单测、数据校验、双模型评测、红队门禁、3210服务重启与浏览器走查。当修改该项目的分诊引擎、模型闸门、医院/案例数据、红队脚本或前端后使用；其他项目勿用。
---

# 诊途 · 改动后验证闭环

项目根即仓库根目录（下文所有命令均在仓库根执行；目录名避免空格与全角字符，命令中仍建议加引号）。Node ≥20、零 npm 依赖、服务端口 3210。
...
- 1.5B 一次性组残留 failedIds：HG-018、HG-020、HG-051（0.5.1 起 HG-041 已过；challenge 集，双模型对照是刻意保留的加分素材，禁止删 1.5B）。
- 红队 RT-018/RT-020（7B 与 1.5B 的"本该弃权却推荐"残留，0.5.1 入口介入后的已知取舍）与 RT-020 词表外语义错配：纯结构核验无法识别，保留为边界说明。
- rules 基线 RT-005/RT-010（口语无匹配转 human）、HG-039~046（口语组 8 条）、dynamic 组 HG-040/041/043（部位选项粒度弃权）均为正常局限，属白名单。
