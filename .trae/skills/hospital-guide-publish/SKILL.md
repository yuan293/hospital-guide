...
description: 诊途 GitHub 仓库的标准发布流程（测试、暂存、提交、绕开失效代理推送、本地远程哈希核验），含历史清洗与删库重建的高危操作规范。当用户要求更新、推送、同步或发布 GitHub 仓库，或要求提交改动时使用。只跑测试或评测时用 hospital-guide-verify。
---
...
- 项目根：仓库根目录（下文所有命令均在仓库根执行，脚本一律用相对路径调用，不依赖绝对路径）
- 远程：https://github.com/yuan293/hospital-guide ，PUBLIC，分支 main，origin 已配置
- 凭据：gh CLI 已 OAuth 登录仓库所有者账号（令牌在系统密钥环）。**禁止**把 token 写进 remote URL 或任何文件
- **代理坑**：若本机 `~/.gitconfig` 配置了不可用的全局代理，裸 `git push` 会失败。推送一律用
  `git -c http.proxy= -c https.proxy= push`（仅本命令生效；repo-local 设空字符串无效，勿再试）
- 网络前提：确保本机可直连 github.com（必要时开启网络加速工具）；curl/gh 如遇 schannel 吊销错误加 `--ssl-no-revoke`
...
| 现象 | 处理 |
|---|---|
| Failed to connect over proxy … | 忘了加空代理参数，重推即可 |
| 443 / Could not resolve | 确认本机可直连 github.com（必要时开启网络加速工具） |
...
3. **关键坑**：强推后 GitHub 对不可达旧提交仍短期保留"精确 SHA 直链"（API/Web 都可达，搜索与浏览已不可见）。要立即 100% 抹除只能**删库同名重建**：
   ...
   - 先记录仓库 description/visibility；`gh repo delete <owner>/<repo> --yes`
   - 立即 `gh repo create <owner>/<repo> --public --description "..."`，再 `git push -u origin main`
