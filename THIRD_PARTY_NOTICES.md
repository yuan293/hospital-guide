...
| 6 | 合成评测集与红队集（数据／自建） | `hospital-guide-synthetic-v1`（81 条）、`redteam-adversarial-v1`（21 条） | 团队自建，随本仓库以 Apache-2.0 开放 | 开源；配套评测脚本可复现全部指标 | 不得将其表述为临床准确率或独立盲测结果 | 案例、真值、红队输入与评测脚本全部由团队编写，描述文本 AI 辅助 | 已核验 |
