  ...
  try {
    ...
    const drawCases = () => {
      ...
      $('#eval-cases').innerHTML = '<p class="field-help">显示 ' + rows.length + ' / ' + mode.metrics.total + ' 条实测（本模式未纳入 ' + mode.metrics.skipped + ' 条） · 模型失败并回退的次数：' + mode.metrics.modelFailures + '</p>' + rows.map(row =>
        ...
        row.trace.map(t => '<li>' + escape(describe(t)) + (t.gain != null ? ' · 信息增益 ' + t.gain + ' 分（并列：' + escape((t.leaders || []).join('、')) + '）' : '') + (t.model?.used ? ' · ' + escape(t.model.matches.map(m => m.evidence + ' → ' + m.keyword).join('；') || '无额外症状') : t.model?.attempted ? ' · 模型失败，规则回退' : ' · 规则处理') + '</li>').join('') + '</ol></details>').join('') + (!rows.length ? '<p class="empty">这一模式没有未符合预期的案例。可取消筛选查看全部。</p>' : '');
    };
